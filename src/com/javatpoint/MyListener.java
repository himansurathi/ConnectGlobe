...
public class MyListener implements ServletContextListener{
	...
	try{
		...
		PreparedStatement ps1=con.prepareStatement("select * from all_objects where object_name = 'FORUMREG'");
		...
		ps.executeUpdate();
		PreparedStatement ps4=con.prepareStatement("CREATE TABLE  FORUMREP(ID NUMBER,COUNTRY VARCHAR2(4000),STATE VARCHAR2(4000),DISTRICT VARCHAR2(4000),POLICE_STATION VARCHAR2(4000),REPORT VARCHAR2(4000),CATEGORY VARCHAR2(4000),IMAGE BLOB,EMAIL VARCHAR2(4000),POSTEDON DATE,CONSTRAINT FORUMREP_PK PRIMARY KEY (ID) ENABLE)");
		ps4.executeUpdate();		
